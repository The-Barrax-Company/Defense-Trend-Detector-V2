...
const UpdateDatabaseButton: React.FC = () => {
  // State to track loading status
  const [isLoading, setIsLoading] = useState<boolean>(false);
  // State to store API response
  const [response, setResponse] = useState<UpdateDatabaseResponse | null>(null);
  ...
  const handleApiCall = async (): Promise<void> => {
    ...
    try {
      ...
      const data: UpdateDatabaseResponse = await result.json();
      setResponse(data);
    } catch (err) {
